...
//The bool functions create (highest) predicates to determine object equality.
bool cpHigher(Advertisement* paa, Advertisement* pab) {
	
	float a1 = 0;
	float a2 = 0;

	if(paa->getBids().size() > 0) {
	
		a1 = paa->getBids().top().getAmount();
	}

	if(pab->getBids().size() > 0) {
	
		a2 = pab->getBids().top().getAmount();
	}
...
//The bool functions create (lowest) predicates to determine object equality.
bool cpLower(Advertisement* paa, Advertisement* pab) {
...
Listing Listing::sort(string field) {
	...
	if(field == "email") {
		...
		//process the By highest sort.
		std::sort(newlisting.begin(), newlisting.end(), cpHigher);
	} else if(field == "lowest") {
		
		//process the By lowest sort.
		std::sort(newlisting.begin(), newlisting.end(), cpLower);
	}
	return newlisting;
}
