...
void displayPostItem (ostringstream &oss, struct in_addr ip, int port) {
	...
	for (CategoryHierarchy::iterator it = categories.begin();
       it != categories.end();
       it++) {
...
void displayShortest (ostringstream &oss, struct in_addr ip, int port) {

	oss << "<B><CENTER>How do they know each other?</CENTER></B><BR>" << endl;
	oss << "<table width=100% border=0>" << endl;
	oss << "<FORM ACTION=" << CLASSIFIED_CGI << " METHOD=POST>" << endl;
	oss << "<INPUT NAME=PORT TYPE=hidden VALUE=" << port << ">" << endl;
	oss << "<INPUT NAME=IP TYPE=hidden VALUE=" << inet_ntoa(ip) << ">" << endl;
	oss << "<tr><td align=right>First email:" << endl;
	oss << "<td><INPUT NAME=path1 TYPE=text SIZE=20 MAXLENGTH=20></tr>" << endl;
	oss << "<tr><td align=right>Second email:" << endl;
	oss << "<td><INPUT NAME=path2 TYPE=text SIZE=20 MAXLENGTH=20></tr>" << endl;
	oss << "<tr><td colspan=2 align=center>";
	oss << "<INPUT NAME=COMMAND VALUE=Path TYPE=submit></tr>" << endl;
	oss << "</FORM></table>" << endl;
