...
void processrequest(istringstream &iss, int connfd, int port) {
  ...
  if (req == "LOGIN") {
	...
	p->addSubCategory(cat.getNumber());
	...
	Listing sorted = advertisements;
	sort(sorted.begin(), sorted.end(), SortBy(field));
	...
	CopyByKeyword cp(keyword, &filtered);
	for_each(advertisements.begin(), advertisements.end(), cp);
	...
	categories.findOfferings (category,  advertisements.begin(),
	...
	Listing filteredByCategory;
	categories.findOfferingsRecursive (category, advertisements.begin(),
						advertisements.end(), filteredByCategory );
	displayListing(connfd, port, filteredByCategory.begin(), filteredByCategory.end());
    ...
    Shortest s (user1);
	vector<string> paths = s.get_path (user2);

    ostringstream oss ("");
    buildpathpage (oss, port, paths);
